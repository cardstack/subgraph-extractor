...
postgresql_my_proc = factories.postgresql_proc(
    load=["tests/resources/example_db.sql"]
)
...
def test_get_subgraph_table_schemas(db_conn):
    ...
    assert subgraph_schemas == {'my_test_subgraph': {'id': 'internalversion1', 'label': 'my_test_subgraph', 'subgraph_deployment': 'SUBGRAPHIPFS', 'subgraph_table_schema': 'sgd1'}} 
...
def test_get_column_types(db_conn, table_schema_name, valid_table_name):
    ...
    assert db_columns == {"block_number": "numeric",
    "amount": "numeric",
    "from_address": "text",
    "to_address": "text"}